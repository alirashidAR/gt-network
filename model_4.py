...
# Your other functions and main code remain unchanged
...
def find_nash_equilibrium():
    ...
    for i, action1 in enumerate(strategies):
        best_response[action1] = strategies[np.argmax(payoffs[i, :])]
    
    for j, action2 in enumerate(strategies):
        if best_response[action2] != strategies[np.argmax(payoffs[:, j])]:
...
def play_game(node1, node2, features):
    ...
    if nash_equilibrium is not None:
        action1 = nash_equilibrium['share'] if np.random.rand() < 0.5 else 'ignore'
        action2 = nash_equilibrium['share'] if np.random.rand() < 0.5 else 'ignore'
    else:
        # Randomly assign if no equilibrium is found
